...
filter( !(is.na(life_expect) | is.na(gdp_percap)))
...
ea_na_75_85 <- longevity %>% filter(region == "East Asia & Pacific" | region == "North America", life_expect >= 75 & life_expect <= 85) %>% arrange(desc(life_expect))
   ```
...
filter( !(is.na(life_expect) | is.na(gdp_percap))) %>%
summarise(total_gdp = sum(gdp_percap * population/1000000000000))
...
5. From `nations`, extract a tibble `p_countries` of dimension 5 x 2
   ...
   ```
??? quasi-bon output -> pas bon p et manque Not classified
View(nations %>% group_by(income) %>% filter(year==2016, life_expect>=70) %>% summarise(count = n()) %>% mutate(prop = count/sum(count)))

p_countries <- nations %>% 
group_by(income) %>% 
filter(year==2016, life_expect>70) %>% 
summarise(count = n()) %>% 
mutate(prop = count/sum(count))

print(p_countries, n = 5)
...
6. Draw multiple horizontal boxplots for the life expectancy against region.
   ...
   ```{r, echo = TRUE}
  nations %>% na.omit %>%
  mutate(region = fct_reorder(region, life_expect, .fun = 'median')) %>%
  ggplot(aes(region,life_expect)) +
  geom_boxplot() +
  coord_flip() +
...
8. To understand how life expectancy evolves over time in different region, draw 
   ...
   ```{r, echo = TRUE}
   nations %>% na.omit %>% ggplot(aes(year, life_expect, group = country)) +
    geom_line(alpha = 0.5) + facet_wrap(~region) + labs(x = "Year", y= "Life Expectancy")
   ```