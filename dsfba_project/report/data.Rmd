...
demostats_2016 <- data3 %>% filter(!(is.na(NEIGHBORHOOD)))

crime_demostats_2016 <- bind_cols(violent_crime_2016_neighborhood, crime_2016_neighborhood, demostats_2016) %>%
  select( everything(), -(NEIGHBORHOOD), -(`Analysis Neighborhood...3`),) %>%
  rename(`Analysis Neighborhood` = `Analysis Neighborhood...1`)
