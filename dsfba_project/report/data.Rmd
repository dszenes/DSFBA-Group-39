...
violent_crime_2016_neighborhood <- crime2003_2020_datesep %>%
  filter(year == 2016, `Incident Category` %in% c("Assault", "Homicide", "Robbery", "Sexual Assault")) %>% count(`Analysis Neighborhood`, name = "Violent crimes") %>%
  select("Violent crimes")
#We select the Assault, Homicide, Sexual Assault and Robbery  categories for a sub-category called Violent crimes 
  
crime_demostats_2016 <- bind_cols(violent_crime_2016_neighborhood, crime_2016_neighborhood, demostats_2016) %>% 
  mutate( `Family Households` = `Family Households` * `Total Population`,
...
crime_demostats_2016 <- crime_demostats_2016[, c(-38,-39)] 



# for the purpose of analysis we have decided to turn all proportions into absolute numbers in order to have a regression that we can better interpret
