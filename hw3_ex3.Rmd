...
fu_separated <- fu %>% separate( date, into = c("year","mon", "day"), sep = "[-]+")

print(fu_separated, n = 5)
...
2. Notice that the data is recorded per game instead of per team. Suppose
   ...
   fu_tidy <- fu_separated %>% pivot_longer(c(home_team,away_team), "team_type", values_to ="team" )
...
3. Now, notice that the two columns `home_score` and `away_score` are
   ...
   fu_team <- fu_tidy %>% mutate(scored = ifelse(team_type == "away_team", away_score, home_score), 
   conceded = ifelse(team_type == "home_team",away_score, home_score), gd = scored-conceded) %>% select(year:day, scored:gd, team, city, country)
...
5. From `fu_city`, let's extract `fu_england`, a tibble containing
   ...
   fu_england <- fu_city %>% group_by(year) %>% filter(team == "England") %>% summarize(scored = mean(scored),
   conceded = mean(conceded), gd = mean(gd)) %>% arrange( desc(gd),  desc(scored))
 
   
   print(fu_england, n = 5)
