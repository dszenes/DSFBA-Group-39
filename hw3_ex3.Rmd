...
```{r}
   ...
   fu_separated <- fu %>%                                         # extract from database
   separate( date, into = c("year","mon", "day"), sep = "[-]+")   # seperate date into year, mon, day variables
   
   kable(head(fu_separated, 1000), caption = "Table with seperated date variable") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box limited to 1st 1000 values
...
```{r}
   ...
   # extract from database, create team_type variable, with home_team and away_team as values
   # and assign the values of home_team and away team to new variable team
   
   fu_tidy <- fu_separated %>%                                                
   pivot_longer(c(home_team,away_team), "team_type", values_to ="team" )      
                                                
   kable(head(fu_tidy, 1000),  caption = "Tidy football data") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box limited to 1st 1000 values
...
```{r}
   ...
   fu_team <- fu_tidy %>%                                                     # extract from database
   mutate(scored = ifelse(team_type == "away_team", away_score, home_score),  # create scored and conceded
   conceded = ifelse(team_type == "home_team",away_score, home_score),        # variables that alternate between
   gd = scored-conceded) %>%                                                  # taking away_score and home_score
   select(year:day, scored:gd, team, city, country)                           # as a value, gd is the difference                                                                               # between the two of them, select                                                                               # desired variables
   
   kable(head(fu_team, 1000), caption = "Data per team with goals scored, conceded and goal difference") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box limited to 1st 1000 values
...
```{r}
   ...
   fu_city <- fu_team %>%           # extract from database
   unite(place, city, country)      # unite city and country into new place variable
   
   kable(head(fu_city, 1000),  caption = "Data by place") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box limited to 1st 1000 values
...
```{r}
   ...
   fu_england <- fu_city %>%                       # extract from database
   group_by(year) %>%                              # group by year
   filter(team == "England") %>%                   # filter for England
   summarize(scored = mean(scored),                # create variables with means
   conceded = mean(conceded),                      # for scored, conceded and gd
   gd = mean(gd)) %>%                              
   arrange( desc(gd),  desc(scored), conceded)     # sort rows in desired order
   
   kable(fu_england,  caption = "English national team") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box
...
```{r}
   ...
   # same as fu_tidy for team_type and team variables, split date variable to extract year
   # scored variable created based on wether matching away_team with away_score and vice versa
   ...
   kable(head(fu_tidy2, 1000),  caption = "A different tidy data table") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
   #output the data cleaning in a scroll box limited to 1st 1000 values
...
```{r}
   ...
   fu1 <- fu_tidy2 %>%                   # extract from database                             
   group_by(team, year) %>%              # group by team and year
   summarize(scored = sum(scored)) %>%   # sum to find goals scored by team per year
   filter(team %in% c("England","Germany","France","Belgium","Italy","Spain"),
   year >= 2000) %>%                     # filter for dwsired teams, matches after from
   ungroup(team) %>%                     # ungroup by team in order to reorder team as factor
   mutate( team = fct_reorder(team, scored, median)) 
   
  kable(fu1,  caption = "Goals scored for 6 national teams") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
  #output the data cleaning in a scroll box
...
```{r}
   ...
   ggplot(fu1, aes( team, scored)) +         # plot team and scored
   geom_boxplot() +                          # in a boxplot
   labs( x = "Goals scored by team", y = "") # with labels
...
```{r}
   ...
   # change tournament variable to factor with relevant levels, filter for relevant teams
   # year, remve NAs for tournament variable
   ...
   kable(fu2,  caption = "Data from for 6 national teams in 3 international tournaments") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
...
```{r}
   ...
   ggplot(fu2, aes(team, fill = tournament ))+        # plot teams and tournament in a barplot
   geom_bar( position=position_dodge())+              # make sure a column is a
   labs( x = "Games played since 2000", y = "")       # tournament per team, add labels
...
```{r}
   ...
   fu3 <- fu_tidy2 %>%                       # extract from database
   filter(tournament =="FIFA World Cup") %>% # filter for World cup 
   group_by(team) %>%                        # group by team
   summarize(n = n_distinct(year)) %>%       # summarize by distinct year to get tournament participations
   filter(n >= 9) %>%                        # at least 9 participations
   arrange(desc(n))                          # ranking of highest
   
   kable(fu3,  caption = "World cup participations by national team") %>% kable_styling() %>% scroll_box(width = "900px", height = "300px")
...
```{r}
   ...
   ggplot(fu3, aes(fct_reorder(team, n), n))+         # plot teams and WC attendances
   geom_bar(stat = "identity")+                       # in a barplot with an x and y variable
   coord_flip() +                                     # make it horizontal
   labs( x = "", y = "World Cup attendance by team")  # labels
