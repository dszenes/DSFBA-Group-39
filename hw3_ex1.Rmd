...
1. Extract a tibble `length_sex_ring` which contains the `sex`, `diameter`,
   ...
   ## Your code goes here
   ```
length_sex_ring <- abalone %>% select(sex, diameter, height, rings)  %>% filter(abalone$length > 0.6) %>% arrange( sex, desc(rings))
...
count_prop <- abalone %>% group_by(sex) %>% summarise( count = n()) %>% mutate (prop = count/sum(count))
...
3. Extract a tibble `mean_max_min` of dimension 3 x 4, which contains the
   ...
   mean_max_min <- abalone%>% group_by(sex) %>% summarize( weight_mean = mean(shucked_weight),
   weight_max = max(shucked_weight), weight_min = min(shucked_weight))
...
4. Extract a tibble `filter_na` of dimension 2963 x 10,
   ...
   filter_na <- abalone %>% filter( diameter == NA | diameter > 0.36) %>% select (index = X, sex, length, diameter, rings, everything())
...
5. Extract a tibble `transmute_abalone` of dimension 4177 x 2,
   ...
   transmute_abalone <- abalone %>% transmute( whole_weight_in_mg = whole_weight*1000, water_weight_in_mg = (whole_weight - (shucked_weight + shell_weight + viscera_weight))*1000)
...
6. Extract a tibble `first_1000_rank` of dimension 1000 x 3,
   ...
   ## Your code goes here
   ```
   first_1000_rank <- abalone %>% mutate( rings_rank = row_number(rings)) %>% select( diameter, rings, rings_rank) %>% filter(rings_rank < 1001) %>% arrange(rings)
...
8. Draw a density plot of `rings`, colored by `sex`.
   ...
   ggplot(abalone, aes(rings, color = sex)) +
   geom_density( )
...
11. Draw a point plot of `diameter` against `rings` with a smooth curve
  ...
  ggplot(abalone, aes(rings, diameter))+
  geom_point()+
