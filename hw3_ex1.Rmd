...
filter(abalone$length > 0.6) %>% 
arrange( sex, desc(rings))

kable(head(length_sex_ring), "simple")
...
4. Extract a tibble `filter_na` of dimension 2963 x 10,
   ...
   filter_na <- 
   abalone %>% 
   filter( is.na(diameter) | diameter > 0.36) %>% select (index = X, sex, length, diameter, rings,               everything())
   
   kable(head(filter_na), "simple")
...
5. Extract a tibble `transmute_abalone` of dimension 4177 x 2,
   ...
   transmute( whole_weight_in_mg = whole_weight*1000, water_weight_in_mg = (whole_weight - (shucked_weight +    shell_weight + viscera_weight))*1000)
   
   kable(head(transmute_abalone), "simple")
...
6. Extract a tibble `first_1000_rank` of dimension 1000 x 3,
   ...
   filter(rings_rank < 1001) %>% 
   arrange(rings)
   
   kable(head(first_1000_rank), "simple")
