...
2. First, let's compute the column that contains the counts across the
   ...
   kable(species_count, "simple")
   
   ```
   
   
print(species_count, n = 1)
...
3. Now let's focus on the numerical variables: for all columns except
   ...
   Max=max), 
   .names = "{.col}_{.fn}")) %>%
   select(contains(c("Min","1st Qu","Median","Mean","3rd Qu","Max")))
   
   kable(summary_stats, "simple")

   ```
   
   print(summary_stats, n = 5)
...
kable(head(summary_long), "simple")
...
5. As for the second step, from `summary_long`, extract a tibble
   ...
   kable(summary_stats_tidy, "simple")
...
6. We are getting closer! The final step: we need to transpose our tibble to
  ...
  pivot_wider(names_from = variable, values_from = value)
   ...
   kable(summary_stats_transposed, "simple")
