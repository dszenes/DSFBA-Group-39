...
3. Now let's focus on the numerical variables: for all columns except
   ...
   ## Your code goes here
   ```
summary_stats <- iris %>% select(-(Species)) %>% summarise(
  across(.cols = everything(), ~ min(.x, na.rm = TRUE), .names = "{.col}_Min"),
  across(.cols = everything(), ~ quantile(.x, na.rm = TRUE), .names = "{.col}_1stQuantile"),
  across(.cols = everything(), ~ median(.x, na.rm = TRUE), .names = "{.col}_Median"),
  across(.cols = everything(), ~ mean(.x, na.rm = TRUE), .names = "{.col}_Mean"),
  across(.cols = everything(), ~ quantile(.x, na.rm = TRUE), .names = "{.col}_3rdQuantile"),
  across(.cols = everything(), ~ max(.x, na.rm = TRUE), .names = "{.col}_Max")
)
