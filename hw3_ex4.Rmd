...
kable(summary_iris) %>% kable_styling(bootstrap_options = c("striped"))
...
kable(species_count) %>% kable_styling(full_width = FALSE)
...
kable(summary_stats) %>% kable_styling() %>% scroll_box(width = "900px", height = "100px")
...
kable(summary_long) %>% kable_styling() %>% scroll_box(width = "500px", height = "200px")
...
```{r}
  ...
  summary_stats_tidy %>% 
  pivot_longer(c(`Min`, `1st Qu`, `Median`, `Mean`, `3rd Qu`, `Max`), names_to = "Stat") %>% 
