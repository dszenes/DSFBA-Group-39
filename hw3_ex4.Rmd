...
kable(summary_iris) %>% kable_styling(bootstrap_options = c("striped")) #choose a particular style for the table
...
kable(species_count) %>% kable_styling(full_width = FALSE) #change the format of the output table
...
kable(summary_stats) %>% kable_styling() %>% scroll_box(width = "900px", height = "100px") #output the new dataset in a scroll box
...
kable(summary_long) %>% kable_styling() %>% scroll_box(width = "500px", height = "200px") #output the new dataset in a scroll box
...
kable(summary_stats_tidy) %>% kable_styling() %>% column_spec(
   1,
   bold = TRUE
...
```{r}
  ...
  pivot_wider(names_from = variable, values_from = value) # use the rows in the variable column in order to make new columns
   ...
   kable(summary_stats_transposed) %>% kable_styling() %>% column_spec(
   1,
   bold = TRUE
) #for the variable column, change the row names in bold black
